...
async def create_order(service: ServiceDependency, payload: ors.OrderCreate):
    try:
        ...
        logger.error(f"Error occurred while creating order: {e}")
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Order has product that does not exist"
        )
